...
class Stacking(object):
    ...
    def fit_predict(self, X, y, T):
        ...
        for i, stacker in enumerate(self.stackers):
            y_predict[:, i, :] = stacker.predict(s_test.reshape((s_test.shape[0],-1)))[:].reshape((-1,1))
            y_predict_weighted += y_predict[:, i, :] * self.weights[i]
        for i in ft:
            y_predict_weighted[i] += ft[i]
...
def CV(model, dl, verbose=1, model_name=None):
    ...
    score = -cross_val_score(model,dl.x_train,dl.y_prep_train,cv=5,scoring='neg_mean_squared_error')
    model.fit(dl.x_train, dl.y_prep_train)
    y_train_predict = dl.ss_y.inverse_transform(model.predict(dl.x_train))
    y_test_predict = dl.ss_y.inverse_transform(model.predict(dl.x_test))
...
def grid_search(model, dl, param_grid, cv=None, verbose=0, model_name=None):
    ...
    grid.fit(dl.x_train, dl.y_prep_train)
...
def varify_on_test(model, dl, verbose=0, model_name=None):
    ...
    model.fit(dl.x_train, dl.y_prep_train.reshape((-1,)))
    y_train_predict = dl.ss_y.inverse_transform(model.predict(dl.x_train))
    y_test_predict = dl.ss_y.inverse_transform(model.predict(dl.x_test))
